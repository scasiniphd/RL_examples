...
class Gridworld:
    ...
    def get_next_state(self, current_position, action):
        state_ = self.state_coord_translation[str(current_position)].copy()

        if action == 0:
            nxtState = [state_[0] - 1, state_[1]]
        elif action == 3:
            nxtState = [state_[0] + 1, state_[1]]
        elif action == 2:
            nxtState = [state_[0], state_[1] - 1]
        else:
            nxtState = [state_[0], state_[1] + 1]
        # if next state legal
        if (nxtState[0] >= 0) and (nxtState[0] <= 2):
            if (nxtState[1] >= 0) and (nxtState[1] <= 3):
                if nxtState != [1, 1]:
                    return self.coord_state_translation[str(nxtState)]
                else:
                    return self.coord_state_translation[str(state_)]
        return self.coord_state_translation[str(state_)]
